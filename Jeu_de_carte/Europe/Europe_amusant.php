...
var id_question = 0;
	...
	var idRandom = getRndInteger(28,32);
	if (idRandom>27 && idRandom<33) {
	$.ajax({
			url:'EuropeAmusantRequetes.php',
			type:'post',
			data:{sow:idRandom},
...
<script>
	...
	function onMapClick(e) {
		popup.setLatLng(e.latlng).setContent("Tu as cliqué sur <br/> " + coordGeoJSON(e.latlng,7)).openOn(map);
        ...
        var ligne = L.polyline(EntreAetB,{color: 'red'}).addTo(map);
			...
			map.setView(pointA, 5);
			L.geoJSON(pays_bas, {color:'green' }).addTo(map);
        var ligne = L.polyline(EntreAetB,{color: 'red'}).addTo(map);
		}
		else{
