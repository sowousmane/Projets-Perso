	...
	<div class="container">
		<div class="row">
			<div class="col-6">
				...
				<div id="amusant" class="amusant">	</div>
...
var id_question = 0;
	...
	var idRandom = getRndInteger(38,42);
	if (idRandom>37 && idRandom<43) {
	$.ajax({
			url:'AsieAmusantRequetes.php',
			type:'post',
			data:{sow:idRandom},
...
<script>
	...
	function onMapClick(e) {
		...
		if( pays =="chine") {
			...
			L.geoJSON(chine, {color:'green' }).addTo(map);
			
			var ligne = L.polyline(EntreAetB,{color: 'red'}).addTo(map);
		}
        else  if( pays =="japon") {
			map.setView(pointA, 5);
			L.geoJSON(japon, {color:'green' }).addTo(map);
        var ligne = L.polyline(EntreAetB,{color: 'red'}).addTo(map);
		}
        else  if( pays =="inde") {
        ...
        else  if( pays =="coree du sud") {
			map.setView(pointA, 6);
			L.geoJSON(coree_du_sud, {color:'green' }).addTo(map);
        var ligne = L.polyline(EntreAetB,{color: 'red'}).addTo(map);
		}
		...
		if(Distance<1000 ){
			...
			document.getElementById("score").innerHTML=(' OMG &#128562; je suis  '+ pays +' &#128516; et ton score est &#128073; '+scoreFunction (Distance))
	...
	var note = 0;
	if( Distance <500 || (Distance >500 && Distance<900 && pays=='inde')){
		note = 10;
		return note;
